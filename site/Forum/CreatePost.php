...
if (!$captcha_completed) {
        exit("Please complete the captcha.");
          ...
          $content = $_POST['content'];
                $tim = time();
$stmt = $db->prepare("INSERT IGNORE INTO `forum`
        (`id`, `author`, `reply_to`, `title`, `content`, `time_posted`, `category`)
        VALUES (NULL, ?, '0', ?, ?, ?, ?)");
$stmt->execute(array($_USER['id'], $title, $content, $tim, $topic));
$insid = $db->lastInsertId(); // error_reporting(E_ALL);
header('location: /Forum/ShowPost.php?id='.$insid);
exit;
                            ...
                            <tr valign="bottom">
                                <td>
								  ...
								  <tr>
                                    ...
                                    <td valign="top" align="right" colspan="2">                                <button id="finish" type="submit">Post</button>
