...
$id = htmlspecialchars((int)$_GET['id'] ?? 0);
...
$fr = $db->prepare("SELECT * FROM forum WHERE reply_to='$id' LIMIT ".$thispagefirstresult.",".$resultsperpage) or die($db->errorInfo());
$fr->execute();
$fq = $db->prepare("SELECT * FROM forum WHERE id='$id'") or die($db->errorInfo());
$fq->execute();

if ($fq->rowCount() < 1) {
  die("<script>document.location = 'Default.aspx'</script>");
}

$fpost = $fq->fetch();

if ($fpost['reply_to'] != 0) {
  header('location: /Forum/ShowPost.aspx?id='.(int)$fpost['reply_to']);
  exit;
}

                    $resultsperpage = 2;
                    $stmt = $db->prepare("SELECT * FROM forum WHERE reply_to=:id");
  $stmt->execute(['id' => $id]);
  $usercount = $stmt->rowCount();
...
$fauthorstmt = $db->prepare("SELECT * FROM users WHERE id=?");
$fauthorstmt->execute([$fpost['author']]);
$fauthor = $fauthorstmt->fetch(PDO::FETCH_ASSOC);

$ftimeago = ("@{$fpost['time_posted']}");

$fauthorpostcountstmt = $db->prepare("SELECT * FROM forum WHERE author=?");
...
$content = $db->quote($content);
    ...
    $rn = time();
    $stmt = $db->prepare("INSERT INTO `forum`
...
$stmt->execute();
      ...
      die("<script>document.location = \"ShowPost.aspx?id=$id\"</script>");
  }
}
...
$fq = $db->prepare("SELECT * FROM forum WHERE category=:topic AND reply_to='0' ORDER BY is_pinned DESC, time_posted DESC LIMIT :limit OFFSET :offset");
...
$stmt_topicsql = $db->prepare("SELECT * FROM topics WHERE id = :topic");
...
$stmt_groupq = $db->prepare("SELECT * FROM forumgroups WHERE id = :category");
...
?>
<link rel="stylesheet" href="api/skins/default/style/default.css" type="text/css">
<div id="Body">
    ...
    <tbody><tr>
                                  ...
                                  <td class="popupMenuSink" width="1px" valign="top" align="left">
                                    <nobr>
                                      ...
                                      <a class="linkMenuSink" href="/Forum/ShowForumGroup.aspx?ForumGroupID=<?php echo (int)$groupium['id']; ?>"><?php echo htmlspecialchars($group['name']); ?></a>
                                    </nobr>
                                  </td>
                                  <td class="popupMenuSink" width="1px" valign="top" align="left">
                                    <nobr>
                                      <span class="normalTextSmallBold">&nbsp;&gt;</span>
                                      <a class="linkMenuSink" href="/Forum/ShowForum.aspx?ForumID=<?php echo (int)$topicscheisse['id']; ?>"><?php echo htmlspecialchars($topics['name']); ?></a>
                                    </nobr>
                                  </td>
                                  <td class="popupMenuSink" width="1px" valign="top" align="left">
                                    <nobr>
                                      <span class="normalTextSmallBold">&nbsp;&gt;</span>
                                      <a class="linkMenuSink" href="/Forum/ShowPost.aspx?id=<?php echo $fpost['id']; ?>"><?php echo filterText(htmlspecialchars($fpost['title'])); ?></a>
                                    </nobr>
                                  </td>
...
<tr>
                  ...
                  <td valign="bottom" align="right">
                    ...
                    <select name="DisplayMode" onchange="location='/Forum/ShowPost.aspx?id=<?=(int)$fpost['id'];?>&amp;mode='+$(this).val();">
                      <option selected="selected" value="Flat">Flat View</option>
                      <option value="Threaded">Threaded View</option>
                    </select>
                    &nbsp;
                    <select name="SortOrder" onchange="location='/Forum/ShowPost.aspx?id=<?=(int)$fpost['id'];?>&amp;SortOrder='+$(this).val();">
        ...
        <td align="left"></td><td align="right"><a class="linkSmallBold" href="">Previous Thread</a>&nbsp;<span class="normalTextSmallBold">::</span>&nbsp;<a class="linkSmallBold" href="">Next Thread</a>&nbsp;</td>
                                                                            ...
                                                                            <tr>
    <th class="tableHeaderText" align="left" height="25" width="100">&nbsp;Author</th><th class="tableHeaderText" align="left" width="85%">&nbsp;Thread: <?php echo filterText(htmlspecialchars($fpost['title'])) ?></th>
  </tr>
        ...
        <td>            <?php 
            $online = ($fauthor['lastseen'] + 300 <= time()) ? "<img src='api/skins/default/images/Offline.gif' border='0'>&nbsp;" : "<img src='api/skins/default/images/Online.gif' border='0'>&nbsp;";
            echo"$online";
            ?><a class="normalTextSmallBold" href="/User.php?ID=<?php echo (int)$fauthor['id'] ?>"><?php echo htmlspecialchars($fauthor['username']) ?></a><br></td>
      </tr>
      <tr>
        <td>
            <a href="/User.php?ID=<?php echo (int)$fauthor['id'] ?>"><img src="../img/user/<?php echo (int)$fauthor['id'] ?>.png" width="64" border="0"></a></td>
      </tr>
      <?php
      ...
      <tr>
        <td><span class="normalTextSmaller"><b>Total Posts: </b><?php echo (int)$fauthorpostcount ?></span></td>
      </tr>
      <tr><td>&nbsp;</td></tr>
                        ...
                        <td class="forumRow" valign="top">
                            <table cellspacing="0" cellpadding="3" border="0" width="100%">
                                <tbody>
                                    <tr>
                                <td class="forumRowHighlight"><span class="normalTextSmallBold"><?php echo filterText(htmlspecialchars($fpost['title'])) ?><a name="112"></a></span><a name="112"><br><span class="normalTextSmaller"> Posted: </span><span class="normalTextSmaller"><?php echo $fpostedat ?></span></a></td>
                              </tr>
                              <tr>
                                  <td colspan="2">
      ...
      <tr>
          <td colspan="2">
              <?php if($fpost['is_locked'] <= 0){?><a href="reply.aspx?id=<?php echo (int)$fpost['id']; ?>&ogid=<?php echo $id; ?>"><img border="0" src="/images/newpost.gif"></a><?php } ?>
...
<table Class="tableBorder" width="100%" cellpadding="3" cellspacing="0">
  <tr>
    <td class="forumRowHighlight" align="center">
      <span class="normalTextSmallBold">Moderate: <?=(int)$fpost['id'];?> [</span>
      <a href="/Forum/Moderate/DeletePost.aspx?PostID=<?=(int)$fpost['id'];?>" id="DeletePost" class="menuTextLink">Delete Post</a> |
...
<?php
    ...
    $authorpostcountstmt = $db->prepare("SELECT * FROM forum WHERE author=:author_id");
    ...
    $author['joindate'] = date_create()->format('d M Y');
                          ...
                          $onlinee = ($author['lastseen'] + 300 <= time()) ? "<img src='api/skins/default/images/Offline.gif' border='0'>&nbsp;" : "<img src='api/skins/default/images/Online.gif' border='0'>&nbsp;";
            echo "
            <tr>
                        <td class='forumRow' valign='top' width='150' nowrap='nowrap'>
                            <table border='0'>
        ...
        <td>$onlinee<a class='normalTextSmallBold' href='/User.php?ID={$author['id']}'>".htmlspecialchars($author['username'])."</a><br></td>
      </tr>
      <tr>
        <td><a href='/User.aspx?ID={$author['id']}'><img src='../img/user/".$author['id'].".png' width='64' border='0'></a></td>
      </tr>
";
...
echo"
      ...
      <tr><td>&nbsp;</td></tr>
                        ...
                        <td class='forumRow' valign='top'>
                            <table cellspacing='0' cellpadding='3' border='0' width='100%'>
                                <tbody>
                                    <tr>
                                <td class='forumRowHighlight'><span class='normalTextSmallBold'>RE: ".filterText(htmlspecialchars($fpost['title']))."<a name='112'></a></span><a name='112'><br><span class='normalTextSmaller'> Posted: </span><span class='normalTextSmaller'><?php echo $fpostedat2 ?></span></a></td>
                              </tr>
                              <tr>
                                  <td colspan='2'>
                                      <span class='normalTextSmall'>".filterText($post['content'])."</span>
                                  </td>
                              </tr>
                              <tr>
        <td colspan='2'><span class='normalTextSmaller'></span></td>
      ...
      <tr>
          <td colspan='2'>
              <a href='/Forum/reply.aspx?id={$post['id']}&ogid={$id}'><img border='0' src='/images/newpost.gif'></a>
              <a href='' style='font-size: 12px;'>Report Abuse</a>
          </td>
      </tr>
                                </tbody>
