...
$check = $db->query("SELECT * FROM forum WHERE reply_to='$id'");
...
$fr = $db->query("SELECT * FROM forum WHERE reply_to='$id'");
...
$fq = $db->query("SELECT * FROM forum WHERE id='$id'");
...
if($_POST['Cancel']){
    ...
    if ($fpost['reply_to'] != 0) {
        $letsgetthis = $db->query("SELECT * FROM forum WHERE id = '".$fpost['reply_to']."'");
...
if($_POST['Delete']){
    if($_POST['DeleteReason'] != ""){
        if($_USER['USER_PERMISSIONS'] == 'Administrator') {
            $delete = $db->query("DELETE FROM forum WHERE id='$id'");
            ...
            if ($fpost['reply_to'] != 0) {
                ...
                if($hasreplies == "Yes"){
                    $delete = $db->query("DELETE FROM forum WHERE reply_to='$id'");
                }
                $whereamiid = $fpost['id'];
...
$topicsql = $db->query("SELECT * FROM topics WHERE id  = '".$fpost['category']."'");
$topicscheisse = $topicsql->fetch(PDO::FETCH_ASSOC);

$groupium1 = $db->query("SELECT * FROM forumgroups WHERE id  = '".$topicscheisse['category']."'");
...
<br><text class="validationWarningSmall"><?php if($_POST['Delete']){ if($_POST['DeleteReason'] == ""){?>You must supply a reason.<?php }} ?>&nbsp;</text>
</td>
              </tr>
            </table>
          </td>
