...
if ($topic >= 9223372036854775807 || $topic <= 0) {
    $topic = 1;
}

$resultsperpage = 15;
$check = $db->prepare("SELECT * FROM forum WHERE category = :topic AND reply_to = '0' ORDER BY is_pinned DESC, time_posted DESC");
$check->execute(['topic' => intval($topic)]); // fixed the shitted code :3
...
$threadsperpage = 20;
$stmt = $db->prepare("SELECT id FROM forum WHERE category = :topic AND reply_to = '0' ORDER BY is_pinned DESC, time_posted DESC");
...
$fq = $db->prepare("SELECT * FROM forum WHERE category = :topic AND reply_to = '0' ORDER BY is_pinned DESC, time_posted DESC LIMIT " . $resultsperpage . ", " . $thispagefirstresult . ";");
$fq->execute(['topic' => $topic]);

$stmt_topicsql = $db->prepare("SELECT * FROM topics WHERE id = :topic");
$stmt_topicsql->execute(['topic' => $topic]);
$topicsql = $stmt_topicsql->fetch(PDO::FETCH_ASSOC);

$topics = $topicsql;

$stmt_groupq = $db->prepare("SELECT * FROM forumgroups WHERE id = :category");
$stmt_groupq->execute(['category' => intval($topicsql['category'])]);
...
*****************************************************/
        ...
        a.userOnlineLinkBold:Visited,
        a.userOnlineLinkBold:Hover,
        a.userOnlineLinkBold:Link {
            font-weight: bold;
            color: #0055E7;
        }
        ...
        a.moderatorOnlineLinkBold:Visited,
        a.moderatorOnlineLinkBold:Hover,
        a.moderatorOnlineLinkBold:Link {
            font-weight: bold;
            color: darkblue;
        }
        ...
        a.adminOnlineLinkBold:Visited,
        a.adminOnlineLinkBold:Hover,
        a.adminOnlineLinkBold:Link {
            font-weight: bold;
            color: black;
        }
                                                    ...
                                                    <table class="tableBorder" cellspacing="1" cellpadding="3" border="0" width="100%">
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                ...
                                                                $poststmt = $db->prepare("SELECT * FROM forum WHERE category=:t AND reply_to='0' ORDER BY is_pinned DESC, time_posted DESC LIMIT :pfr, :rpp");
$poststmt->bindValue(':t', $topic, PDO::PARAM_INT);
$poststmt->bindValue(':pfr', $thispagefirstresult, PDO::PARAM_INT);
$poststmt->bindValue(':rpp', $resultsperpage, PDO::PARAM_INT);
$poststmt->execute();

                                                                while ($post = $poststmt->fetch(PDO::FETCH_ASSOC)) {
                                                                    $authorstmt = $db->query("SELECT * FROM users WHERE id='{$post['author']}'");
                                                                    $author = $authorstmt->fetch(PDO::FETCH_ASSOC);
                                                                    $repliesstmt = $db->query("SELECT id FROM forum WHERE reply_to='{$post['id']}'");
                                                                    $replies = $repliesstmt->rowCount();
                                                                    if ($replies > 0) {
                                                                        $lastreplystmt = $db->query("SELECT * FROM forum WHERE reply_to='{$post['id']}' ORDER BY id DESC LIMIT 1");
                                                                        ...
                                                                        $lrauthorstmt = $db->query("SELECT * FROM users WHERE id='{$lastreply['author']}'");
                                                                        $lrauthor = $lrauthorstmt->fetch(PDO::FETCH_ASSOC);
                                                                        $lrstring = "<b>" . forumtime($post['time_posted']) . "</b> <br> by <a href=\"/Forum/User/UserProfile.aspx?UserName=".htmlspecialchars($author['username'])."\">".htmlspecialchars($author['username'])."</a>";
                                                                        if ($post['is_pinned'] > 0) {
                                                                            $lrstring = "<b>Pinned Post</b> <br> by <a href=\"/Forum/User/UserProfile.aspx?UserName=".htmlspecialchars($author['username'])."\">".htmlspecialchars($author['username'])."</a>";
                                                                        }
                                                                    }
                                                                    ...
                                                                    if ($post['is_pinned'] > 0) {
                                                                        if ($post['is_locked'] > 0) {
                                                                            $icon = "topic-pinnedandlocked_notread.gif";
                                                                        }
                                                                    }
                                                                    ...
                                                                    $lstmt = $db->query("SELECT * FROM forum WHERE reply_to='{$post['id']}' ORDER BY id DESC LIMIT 1");
...
<img title='Post' src='api/skins/default/images/$icon' border='0'>
</td>
<td class='forumRow' height='25'><a class='linkSmallBold' href='/Forum/ShowPost.aspx?id=".(int)$post['id']."'>".filterText(htmlspecialchars($post['title']))."</a>
</td>
<td class='forumRowHighlight' align='left' width='100'>
  &nbsp;
  <a class='linkSmall' href='/User.aspx?ID=".(int)$author['id']."'>".htmlspecialchars($author['username'])."</a>
</td>
<td class='forumRowHighlight' align='center' width='50'>
  <span class='normalTextSmaller'>".(int)$replies."</span>
</td>
...
<br>by </span>
<a class='linkSmall' href='/User.php?id=".(int)$lrauthor['id']."'>".htmlspecialchars($lrauthor['username'])."</a>
<a href='/User.php?id=".htmlspecialchars($lrauthor['id'])."'>
...
<br>by </span>
<a class='linkSmall' href='/User.php?id=".(int)$author['id']."'>".htmlspecialchars($author['username'])."</a>
<a href='/User.php?ID=".(int)$author['id']."'>
<img border='0' src='/Forum/api/skins/default/images/icon_mini_topic.gif'></a></td>
  </tr>";
                                                                    }
                                                                }
...
$poststmt->execute();
                        ...
                        $paget = htmlspecialchars($_GET['page']);
                    }
