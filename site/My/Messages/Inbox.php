...
if($_POST['read']){
$ok = $db->prepare("SELECT * FROM messages WHERE `id` = :read");
$ok->bindParam(':read', (int)$_POST['read'], PDO::PARAM_INT);
$ok->execute();
$okletsgo = $ok->fetch();

if($okletsgo['user_to'] == $_USER['id']){
    $yeet = $db->prepare("UPDATE messages SET `readto` = '1' WHERE `id` = :read");
...
$yeet = $db->prepare("UPDATE messages SET `readto` = '1' WHERE `user_to` = :user_to");
...
<div id="Body">
  ...
  <table cellspacing="0" cellpadding="3" border="0" id="ctl00_cphRoblox_InboxGridView" style="width:726px;border-collapse:collapse;">
    <tr class="InboxHeader">
     ...
     <?php
                     $sql = "SELECT * FROM messages WHERE user_to = :user_to ORDER BY `id` DESC";
$stmt = $db->prepare($sql);
...
if ($resultCheck > 0) {
    foreach ($result as $row) {
        $creatorq = $db->prepare("SELECT * FROM users WHERE id = :user_id");
